...
def getObjectForNode(nodeIndex, oldObjects):
    for index, object in enumerate(oldObjects):
        if object.fNodeIndex == nodeIndex:
            return (index, object)
    return (-1, None)

def upgradeObject(oldObject, oldObjects, oldNodes):
    newObject = {}
    ...
    oldNode = oldNodes[oldObject.fNodeIndex]

    for index, node in enumerate(oldNodes):
        if node.fParent == oldNode.fParent and index != oldObject.fNodeIndex:
            objectIndex, object = getObjectForNode(index, oldObjects)
            newObject["nextSibling"] = objectIndex
            break
...
def upgradeNode(oldNode, oldNodeIndex, oldNodes, oldObjects):
    newNode = {}
    ...
    for object in oldObjects:
        if object.fNodeIndex == oldNodeIndex:
            newNode["firstObject"] = oldObjects.index(object)
            break
    # just to give us more work
    for index, node in enumerate(oldNodes):
        if node.fParent == oldNodeIndex:
            newNode["firstChild"] = index
            break
    for index, node in enumerate(oldNodes):
        if node.fParent == oldNode.fParent and index != oldNodeIndex:
            newNode["nextSibling"] = index
            break
    return newNode
